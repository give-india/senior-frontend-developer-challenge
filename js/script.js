  ...
  methods: {    
    formatInput: function(){ // Method Called up submitting User Input to format the user input to JSON Object.
      ...
      if(this.input && this.patch){
        ...
        if(this.isJson(this.input)) // Checking if the Input Text is a valid JSON or not.
          this.jsonObj = JSON.parse(this.input); // Assigning the JSON Value;                  
        else
          alert('Input Base Object is not a valid JSON');

        if(this.isJson(this.patch)) // Checking if the Input Text is a valid JSON or not.
          this.jsonPatch = JSON.parse(this.patch);        
        else
          alert('Input JSON Patch is not a valid JSON');        
    ...
    showFailure(){      
      document.getElementById('testStatus').innerHTML = 'Test Status: <span class="danger">Test Failed</span>';
    },

    showSuccess(){      
      document.getElementById('testStatus').innerHTML = 'Test Status: <span class="new">Test Succeeded</span>';
    },

    showNothing(){      
      document.getElementById('testStatus').innerHTML = 'Test Status: ';
    }    
